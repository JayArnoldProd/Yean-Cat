...
rm -rf code_text/*
mkdir -p code_text || { echo "Failed to create directory"; exit 1; }
...
find . -type f | while read -r file; do
    ...
    mkdir -p "code_text/$dir" || { echo "Failed to create directory $dir"; exit 1; }
    ...
    cp "$file" "$dest_file" || { echo "Failed to copy $file to $dest_file"; exit 1; }
    echo "Copying $file to $dest_file"
done

# Call generate_hierarchy.sh to generate the hierarchy files
./generate_hierarchy.sh || { echo "Hierarchy generation failed"; exit 1; }
