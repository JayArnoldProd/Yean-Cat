...
all_files=$(find . -type f ! -path "./$BACKUP_DIR/*" ! -path "./.git/*" ! -path "./YEAN CAT/*" ! -path "./YEAN CAT SERVER/sounds/*" ! -path "./YEAN CAT SERVER/sprites/*" ! -path "./folder_backups/*" ! -name ".DS_Store")
...
# Step 13: Generate project hierarchy files
tree -a "YEAN CAT" -I 'node_modules|*.DS_Store' -o client_project_hierarchy.txt
grep -v "client_project_hierarchy.txt" client_project_hierarchy.txt > client_project_hierarchy.txt

tree -a "YEAN CAT SERVER" -I 'sounds|sprites|node_modules|*.DS_Store' -o server_project_hierarchy.txt
grep -v "server_project_hierarchy.txt" server_project_hierarchy.txt > server_project_hierarchy.txt

tree -a . -I '.git|node_modules|code_text|folder_backups|*.DS_Store' -o python_heroku_gpt_server_hierarchy.txt
grep -v "python_heroku_gpt_server_hierarchy.txt" python_heroku_gpt_server_hierarchy.txt > python_heroku_gpt_server_hierarchy.txt
