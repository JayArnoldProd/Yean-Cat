#!/bin/bash

echo "Starting backup script..."

# Define directories and files to exclude from the backup
exclude_files=(".git" "node_modules" "__pycache__" "*.pyc" ".DS_Store" "tests" ".env" "*.yyp" "*.yy" "*.otf" "*.ttf" "*.pdf" "*.zip" "*.csv" "*.txt")

# Function to check if a file should be excluded
should_exclude() {
    local file=$1
    for exclude in "${exclude_files[@]}"; do
        if [[ $file == $exclude || $file == *$exclude ]]; then
            return 0
        fi
    done
    return 1
}

# Delete all files in the code_text directory
rm -rf code_text/*
mkdir -p code_text

echo "Files deleted."

# Copy files to code_text directory, creating a .txt file for each
find . -type f ! -path "./code_text/*" ! -path "./folder_backups/*" | while read -r file; do
    if should_exclude "$file"; then
        continue
    fi

    # Get the file extension
    extension="${file##*.}"
    base_name=$(basename "$file")
    dest_file="code_text/${base_name}.${extension}.txt"

    # Copy the file content to the .txt file
    cp "$file" "$dest_file"
    echo "Copying $file to $dest_file"
done

# Create a master backup for the entire code_text directory
backup_file="folder_backups/code_text_backup.txt"
echo "Creating master backup for code_text at $backup_file"
> "$backup_file"
find code_text -type f | while read -r file; do
    echo "Adding $file to $backup_file"
    cat "$file" >> "$backup_file"
done

echo "Backup completed successfully!"
