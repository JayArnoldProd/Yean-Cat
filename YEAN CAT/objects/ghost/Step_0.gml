...
if global.pause=1 {
	...
	if movetype=9 {
	if speed=0 {
		speed=clamp(5+(master.speedd/15),0,20);
	}
	}
}
...
if movetype=9 {
	...
	var way=choose(-1,1)
if way=-1 {
x=room_width/2+random_range(-2000*master.camscale/2,2000*master.camscale/2)
y=choose(-1,1)*1300*master.camscale/2+room_height/2
} else {
y=room_height/2+random_range(-1080*master.camscale/2,1080*master.camscale/2)
x=choose(-1,1)*2250*master.camscale/2+room_width/2
}
if movetype=9 {
	directional=1
if directional=1 {
	...
	speed=clamp(spd*(4+(master.speedd+10)/30),0,15);
		}
	}
} else {
spd=clamp(spd*(.5+(master.speedd+10)/30),0,40);
}
}
...
if instance_exists(yeancat) {
	...
	y2=(varispeed*((master.speedd+10)/30)*spd/(clamp(abs(yeancat.x-x),1920/2,1920*2)/(1920*2)))*sign(yeancat.y-y)*(abs(yeancat.x-x)/(1920*2))
	
y+=(y2*(1-oblivious)*(1-master.vanish)+yyy*(1-((1-(master.vanish))/(oblivious+1)-oblivious/2)))*(global.room_speed_set/1000000*delta_time)
x-=abs(yyy)*(global.room_speed_set/1000000*delta_time)
	}
	if movetype=1  or movetype=6{
	y2=(varispeed*((master.speedd+10)/30)*spd/(clamp(abs(yeancat.x-x),1920/2,1920*2)/(1920*2)))*sign(yeancat.y-y)*(abs(yeancat.x-x)/(1920*2))

y+=(y2*(1-oblivious)*(1-master.vanish)+yyy*(1-((1-(master.vanish))/(oblivious+1)-oblivious/2)))*(global.room_speed_set/1000000*delta_time)
x+=(abs(yyy)/2)*(global.room_speed_set/1000000*delta_time)
	}
	if movetype=2 or movetype=5 {

	y2=(varispeed*((master.speedd+10)/30)*spd/(clamp(abs(yeancat.y-y),1080/2,1080*2)/2160))*sign(yeancat.x-x)*(abs(yeancat.y-y)/2160)

x+=(y2*(1-oblivious)*(1-master.vanish)+yyy*(1-((1-(master.vanish))/(oblivious+1)-oblivious/2)))*(global.room_speed_set/1000000*delta_time)
y-=(abs(yyy)/2)*(global.room_speed_set/1000000*delta_time)
	}
