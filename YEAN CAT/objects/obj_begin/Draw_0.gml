...
draw_sprite_ext(spr_begin, 2,x,y,1*scale,1*scale,0,col,1);
//    surface_set_target(_surf22);
offset=0
	draw_sprite_ext(spr_begin, 1,x,y,1*scale,1*scale,0,make_color_hsv(0,0,hover),1);
...
//if sf=1 {
	...
	offset=0
	draw_sprite(spr_begin, 0, _sw/2,_sh/2);
...
//    surface_set_target(_surf22);
offset=0
draw_sprite_ext(spr_begin, 1, x,y,1*scale,1*scale,0,make_color_hsv(0,0,clamp(255-hover,0,255)),1);

draw_sprite_ext(spr_begin, 1, x,y,1*scale,1*scale,0,make_color_hsv((colcol+timee+offset*14) % 256 ,(255 / 2) * (1 + sin((timee / 100+offset*pi/8) * 2 * pi))/2+127,.5*(255 / 2) * (1 - cos((timee / 100+(7-offset)*pi/8) * pi)) + 255 / 2),hover/255);
...
draw_sprite_ext(spr_begin, 3,x,y,1*scale,1*scale,0,c_white,(255-hover)/255);
