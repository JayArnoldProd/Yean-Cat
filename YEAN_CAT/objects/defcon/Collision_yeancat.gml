...
soundscream = audio_play_sound_on(s_emit,master.charscreams[master.charselect],0,0);
if master.charselect !=0 {
	//mute scream
	//replace this with individual sounds for each character
	audio_sound_gain(soundscream,0,0);
}
	}
	if other.def<=variant {
other.def=variant*master.pickuppotency
	}
}
if itemidd!=-1 {
